...
app.get("/api/results/driver/:driverRef", async (req, res) => {
  try {
    ...
    // Query the results table to get all results for the specified driverId
    const { data: resultsData, error: resultsError } = await supabase
      .from("results")
      .select("*")
    ...
    // Extract unique raceIds and constructorIds from the results data
    const raceIds = [...new Set(resultsData.map((result) => result.raceId))];
    const constructorIds = [
      ...new Set(resultsData.map((result) => result.constructorId)),
    ];

    // Query the races and constructors tables to get additional information
    const { data: racesData, error: racesError } = await supabase
      .from("races")
      .select("name, round, year, date")
      .in("raceId", raceIds);
    ...
    // Create a map of races and constructors for efficient lookup
    const racesMap = {};
    racesData.forEach((race) => {
      racesMap[race.raceId] = race;
    });
...
app.get("/api/qualifying/:raceId", async (req, res) => {
  try {
    ...
    // Query the qualifying table to get all qualifying results for the specified raceId
    const { data: qualifyingData, error: qualifyingError } = await supabase
      .from("qualifying")
      .select("*")
      .eq("raceId", raceId);
    ...
    if (!qualifyingData || qualifyingData.length === 0) {
      return res
        .status(404)
        .json({ error: "Qualifying results not found for the specified" });
    }

    // Extract unique driverIds and constructorIds from the qualifying data
    const driverIds = [
      ...new Set(qualifyingData.map((result) => result.driverId)),
    ];
    const constructorIds = [
      ...new Set(qualifyingData.map((result) => result.constructorId)),
    ];

    // Query the drivers table to get additional information about the drivers
    const { data: driversData, error: driversError } = await supabase
      .from("drivers")
      .select("driverRef", "code", "forename", "surname")
      .in("driverId", driverIds);
    ...
    // Create a map of drivers for efficient lookup
    const driversMap = {};
    driversData.forEach((driver) => {
      driversMap[driver.driverId] = driver;
    });

    // Query the constructors table to get additional information about the constructors
    const { data: constructorsData, error: constructorsError } = await supabase
      .from("constructors")
      .select("name", "constructorRef", "nationality")
      .in("constructorId", constructorIds);
    ...
    // Format the qualifying results with driver and constructor details
    const formattedQualifyingResults = qualifyingData.map((result) => ({
      position: result.position,
      driver: driversMap[result.driverId],
      constructor: constructorsMap[result.constructorId],
    }));

    // Sort the qualifying results by position in ascending order
    formattedQualifyingResults.sort((a, b) => a.position - b.position);

    res.json(formattedQualifyingResults);
  } catch (error) {
